...
| 状态 + 出口 | 第一行圆点 + 文字（文案判定与卡片一致）；第二行是实际出口 IP，悬停显示网关，直连用告警色 |
...
- **响应式**：列表宽度 ≤1100px 拆成两行（账号 / 状态 / 操作在上，健康度与额度在下）；≤640px 单列堆叠
- 行内不展示套餐 chip、重置积分明细和「重置额度」按钮，这些低频信息留在卡片视图和额度页
- **出口的读取方式**：列表接口 `GET /auth-files` 不返回 `proxy_url`，只能和「设置」弹窗一样下载凭证文件本身读取
  （`useAuthFileProxies`）。只读当前页、并发 4，按「文件名 + 修改时间 + 大小」缓存，改了代理后文件重写、自动重新读取。
  凭证文件里有令牌，所以解析后**只保留出口 IP、网关、协议**（`proxyInfo.ts`），代理账号密码与文件其余内容一概不保留；
  测试见 `tests/authFileProxyInfo.test.ts`
- **出口的判定顺序**与 CLIProxyAPI 一致：凭证自身 `proxy_url` → `config.yaml` 全局 `proxy-url` → 直连。
  Decodo 这类「用户名锁定出口」（`user-<账号>-ip-<出口IP>`）取用户名里的 IP，而不是网关域名
- **直连用告警色**：在面板上删掉重加凭证会丢失 `proxy_url`，账号悄悄退回服务器自身 IP 出网，这是最需要一眼看出的情况
- **断点用容器查询**（`.listRows` 声明 `container: auth-file-list`），按列表实际宽度而不是视口宽度切换：
  侧边栏展开会占掉约 250px，按视口判断会在列表很窄时仍用单行布局而溢出
