...
describe('Devin OAuth login UI', () => {
  // Stallion-X 不对外提供 Devin 登录入口：上游的提供商定义与登录逻辑保留，
  // 但 OAuthPage 的 HIDDEN_PROVIDER_IDS 会在渲染前过滤掉该卡片。
  test('does not render the Devin login card while other providers stay visible', () => {
    ...
    expect(markup).not.toContain('Devin OAuth');
    expect(markup).not.toContain('Start Devin Login');
    expect(markup).toContain('Codex OAuth');
    expect(markup).toContain('xAI OAuth');
  });
